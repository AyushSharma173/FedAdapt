...
class Aggregator(job_api_pb2_grpc.JobServiceServicer):
    ...
    def init_control_communication(self):
        ...
        print(f"self.experiment_mode: {self.experiment_mode}")
        # self.experiment_mode = commons.SIMULATION_MODE
        if self.experiment_mode == commons.SIMULATION_MODE:
            print(f"In SIMULATION MODE!")
            num_of_executors = 0
    ...
    def round_completion_handler(self):
        ...
        # 0) Profile current clients
        profiles = {}   # client_id -> dict(computation_per_step, communication_per_byte)
        for client in self.sampled_participants:
            # ask client_manager for its timing on the CURRENT args
            base = self.client_conf.get(client, self.args)
            cost = self.client_manager.get_completion_time(
                client,
                batch_size=base.batch_size,
                local_steps=1,           # profile per step
                upload_size=self.model_update_size,
                download_size=self.model_update_size,
            )
            profiles[client] = {
                "t_step": cost["computation"],
                "t_comm": cost["communication"] / self.model_update_size
            }



        # 1) Co-optimize for a small grid
        new_conf = {}
        model_bits = self.model_update_size
        for client, prof in profiles.items():
            best = None
            # grid of local steps
            for k in [1, self.args.local_steps]:
                # grid of compression ratios
                for c in [1.0, 0.5, 0.25]:
                    T = k * prof["t_step"] + c * model_bits * prof["t_comm"]
                    if best is None or T < best[0]:
                        best = (T, k, c)

            # ⚡ HERE: instead of a dict, create a Namespace with all needed attrs
            new_conf[client] = Namespace(
                # required for tictak_client_tasks:
                batch_size=self.args.batch_size,
                local_steps=best[1],
                # if you later need to read compression in create_client_task/get_client_conf
                compression=best[2],
                # you can also pull in any other defaults you’ll need:
                # e.g. num_participants, clip_bound, whatever your clients expect
                num_participants=self.args.num_participants,
                clip_bound=self.args.clip_bound,
                # …and so on…
            )

        # 2) Store for use in create_client_task() / get_client_conf()
    ...
    def get_client_conf(self, client_id):
        ...
        # tuned = self.client_conf.get(client_id, {})
        # base.update(tuned)
        # return base

        # 2) pull the tuned config -- may be a dict or a Namespace
        tuned = self.client_conf.get(client_id, {})

        # 3) if it's a Namespace, turn it into a dict
        if isinstance(tuned, Namespace):
            tuned = vars(tuned)

        # 4) now safe to update
        base.update(tuned)

        logging.info(f"  → client {client_id} tuned conf: {tuned}")
