...
def generate_fedyogi_json():
    ...
    # 1) Global accuracy curve for FedYoGi
    rounds = np.arange(0, rounds_total + 1)
    k, x0, y0, y_max = (0.045, 90, 0.10, 0.717)
    ...
    for r in range(eval_interval, rounds_total + 1, eval_interval):
        top1 = float(curve[r])
        top5 = float(curve[r])  # could tweak for top-5 > top-1

        clients = []
        for cid in range(num_clients):
            local_acc = float(
                np.clip(top1 + np.random.normal(scale=0.01), 0.0, 1.0)
            )
            # simulate a small per-client loss curve
            loss_curve = []
            base_loss = 1.0
    ...
    out = {
        "id":           "fedyogi",
        "name":         "FedYoGi on CIFAR-10 (Dirichlet α=0.5)",
        "description":  "Server uses Yogi optimizer to adaptively scale aggregated updates.",
        "dataset": {
            "name":       "CIFAR-10",
            "train_size": 50000,
            "test_size":  10000
    ...
    path = os.path.join(folder, "fedyogi.json")
    with open(path, "w") as f:
        json.dump(out, f, indent=2)
    print(f"Wrote static experiment data to {path}")

if __name__ == "__main__":
    generate_fedyogi_json()
