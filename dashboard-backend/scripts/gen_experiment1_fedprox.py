...
def generate_fedprox_json():
    ...
    # 1) Global accuracy curve for FedProx
    ...
    for r in range(eval_interval, rounds_total + 1, eval_interval):
        top1 = float(curve[r])
        top5 = float(curve[r])  # you could make top5 > top1 if you like

        # per-client metrics
        clients = []
        for cid in range(num_clients):
            # local accuracy: small jitter around the global
            local_acc = float(
                np.clip(top1 + np.random.normal(scale=0.01), 0.0, 1.0)
            )
            # simulate a little loss‐curve of length 10
            loss_curve = []
            base_loss = 1.0
    ...
    path = os.path.join(folder, "fedprox.json")
